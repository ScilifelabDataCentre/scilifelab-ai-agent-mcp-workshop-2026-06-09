...
def process_compounds(compounds_list, identifier, confidence_threshold=8, assay_type_in=['B', 'F'], pchembl_value_gte=6):
    ...
    -------
        Four DataFrames containing:
            ...
            - compound_status: Per-compound processing status/diagnostics
    """
    ...
    compound_status_rows: list[dict] = []
    ...
    for i, (index, row) in enumerate (compounds_list.iterrows(), start=1):
        try:
            ...
            try:
                ...
                lookup_errors.append(f"pubchem_get_cid: {e}")
            try:
                drug_schembl = surechembl_get_id(compound, identifier_type)
            except Exception as e:
                lookup_errors.append(f"surechembl_get_id: {e}")
            ...
            compound_status_rows.append(
                {
                    "input_identifier": compound,
                    "identifier_type": identifier_type,
                    "chembl_id": chembl_id,
                    "pubchem_cid": drug_cid,
                    "surechembl_id": drug_schembl,
                    "drug_info_rows": len(drug_info),
                    "drug_assay_rows": len(drug_assay),
                    "drug_moa_rows": len(drug_MoA),
            ...
            compound_status_rows.append(
                {
                    "input_identifier": row.get(identifier_column, None),
                    "identifier_type": identifier_type,
                    "chembl_id": None,
                    "pubchem_cid": None,
                    "surechembl_id": None,
        ...
        pbar.update(1)  
    # Return the DataFrames containing processed information
    compound_status = pd.DataFrame(compound_status_rows)
    return all_drug_info, all_drug_assay, all_MoA, compound_status
